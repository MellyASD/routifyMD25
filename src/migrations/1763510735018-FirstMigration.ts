...
export class FirstMigration1763510735018 implements MigrationInterface {
    name = 'FirstMigration1763510735018'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`transports\` (\`id\` int NOT NULL AUTO_INCREMENT, \`type\` enum ('bus', 'taxi', 'metro', 'bicycle', 'walking', 'private_car') NOT NULL, \`origin\` varchar(255) NOT NULL, \`destination\` varchar(255) NOT NULL, \`distance\` decimal(10,2) NOT NULL, \`duration\` int NOT NULL, \`cost\` decimal(10,2) NOT NULL, \`comfort\` decimal(5,2) NOT NULL, \`reliability\` decimal(5,2) NOT NULL, \`notes\` varchar(255) NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`comparisonId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`transports\` ADD CONSTRAINT \`FK_bc3df8e7bba16ca3ba0d0a83e99\` FOREIGN KEY (\`comparisonId\`) REFERENCES \`comparisons\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`transports\` DROP FOREIGN KEY \`FK_bc3df8e7bba16ca3ba0d0a83e99\``);
